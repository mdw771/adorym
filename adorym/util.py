...
def calculate_pad_len(this_obj_size, probe_pos, probe_size):
    ...
    if max(probe_pos[:, 0]) + probe_size[0] + pad_arr[0, 0] > this_obj_size[0]:
        pad_len = (
            int(max(probe_pos[:, 0])) + probe_size[0] - this_obj_size[0] + pad_arr[0, 0]
        )
        pad_arr[0, 1] = pad_len
    if max(probe_pos[:, 1]) + probe_size[1] + pad_arr[1, 0] > this_obj_size[1]:
        pad_len = (
            int(max(probe_pos[:, 1])) + probe_size[1] - this_obj_size[1] + pad_arr[1, 0]
        )
        pad_arr[1, 1] = pad_len
    return pad_arr
